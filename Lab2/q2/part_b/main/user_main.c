...
#define LOW_PRIORITY		8								//Low priority level for tasks
#define MEDIUM_PRIORITY		9								//Medium priority level for tasks
#define HIGH_PRIORITY		10								//High priority level for tasks
...
void app_main(void)
{
    ...
    xTaskCreate(gpio_status_msg, "task1", 2048, NULL, LOW_PRIORITY, NULL);
    ...
    if( xMutex != NULL) 
    {
        ...
        xTaskCreate(gpio_pin_on, "task2", 2048, NULL, MEDIUM_PRIORITY, NULL);
        //Create task to turn the gpio pin off
        xTaskCreate(gpio_pin_off, "task3", 2048, NULL, HIGH_PRIORITY, NULL);
    }
